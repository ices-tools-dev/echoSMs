...
def fetch_shape_data(specimen_id, progress_callback=None):
    ...
    Returns a dict compatible with ShapeViewerComboApp: {'data': json_dict, 'model_type': 'DATASTORE'}
    """
    ...
    if cache_path.exists():
        try:
            with open(cache_path, 'r', encoding='utf-8') as f:
                data = json.load(f)
                return {'data': data, 'model_type': 'DATASTORE', 'source': f"Cache: {specimen_id}"}
        except Exception as e:
    ...
    try:
        ...
        return {'data': data, 'model_type': 'DATASTORE', 'source': f"Web: {specimen_id}"}
    except Exception as e:
