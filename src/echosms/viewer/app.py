...
from ..utils_datastore import plot_shape_outline, outline_from_krm, outline_from_dwba
...
class ShapeViewerComboApp:
    def __init__(self, root):
        ...
        # Source Selection
        ttk.Label(self.left_pane, text="Shape Source:").pack(anchor=tk.W, pady=(0, 5))
        source_frame = ttk.Frame(self.left_pane)
        source_frame.pack(fill=tk.X, pady=(0, 10))
        ttk.Radiobutton(source_frame, text="Web Datastore", variable=self.source_mode, value="Web", command=self.toggle_source_ui).pack(side=tk.LEFT, padx=(0, 10))
        ttk.Radiobutton(source_frame, text="Load File", variable=self.source_mode, value="File", command=self.toggle_source_ui).pack(side=tk.LEFT, padx=(0, 10))
        ttk.Radiobutton(source_frame, text="Regular Shape", variable=self.source_mode, value="Regular", command=self.toggle_source_ui).pack(side=tk.LEFT)

        # --- File Loading UI ---
        self.file_ui_frame = ttk.Frame(self.left_pane)
        # self.file_ui_frame.pack(fill=tk.X) # Managed by toggle_source_ui
        
        ttk.Label(self.file_ui_frame, text="Select Shapes File (.toml):").pack(anchor=tk.W, pady=(0, 5))
        ...
        ttk.Label(self.file_ui_frame, text="Select Organism:").pack(anchor=tk.W, pady=(0, 5))
        self.shape_selector = ttk.Combobox(self.file_ui_frame, state="readonly", width=30)
        self.shape_selector.pack(fill=tk.X)
        self.shape_selector.bind("<<ComboboxSelected>>", self.refresh_plot)

        # --- Web Datastore UI ---
        self.web_ui_frame = ttk.Frame(self.left_pane)
        
        ttk.Label(self.web_ui_frame, text="Online Shapes Source:").pack(anchor=tk.W, pady=(0, 5))
        ttk.Label(self.web_ui_frame, text="Official EchoSMs Repository (GitHub)", font=("", 8, "italic")).pack(anchor=tk.W, pady=(0, 10))
        
        self.fetch_btn = ttk.Button(self.web_ui_frame, text="Connect & Fetch Shapes", command=self.fetch_web_shapes)
        self.fetch_btn.pack(fill=tk.X, pady=(0, 10))
        
        self.web_status_var = tk.StringVar(value="Not Connected")
        self.web_status_label = tk.Label(self.web_ui_frame, textvariable=self.web_status_var, anchor=tk.W)
        self.web_status_label.pack(fill=tk.X, pady=(0, 5))

        ttk.Label(self.web_ui_frame, text="Select Organism (Web):").pack(anchor=tk.W, pady=(0, 5))
        self.web_shape_selector = ttk.Combobox(self.web_ui_frame, state="readonly", width=30)
        self.web_shape_selector.pack(fill=tk.X)
        self.web_shape_selector.bind("<<ComboboxSelected>>", self.on_web_shape_selected)

        # --- Regular Shape Generator UI ---
        self.regular_ui_frame = ttk.Frame(self.left_pane)
        # Don't pack initially
        
        ttk.Label(self.regular_ui_frame, text="Select Shape Type:").pack(anchor=tk.W, pady=(0, 5))
        self.reg_shape_type = tk.StringVar()
        self.reg_shape_selector = ttk.Combobox(self.regular_ui_frame, textvariable=self.reg_shape_type, state="readonly", width=30)
        self.reg_shape_selector['values'] = ["Sphere", "Prolate Spheroid", "Cylinder", "Bent Cylinder"]
        self.reg_shape_selector.current(0)
        self.reg_shape_selector.pack(fill=tk.X, pady=(0, 10))
        self.reg_shape_selector.bind("<<ComboboxSelected>>", self.update_param_fields)
        ...
        self.gen_btn = ttk.Button(self.regular_ui_frame, text="Generate Shape", command=self.generate_regular_shape)
        ...
        ttk.Radiobutton(save_fmt_frame, text="Auto", variable=self.save_format_var, value="Auto").pack(side=tk.LEFT, padx=5)
        ttk.Radiobutton(save_fmt_frame, text="KRM", variable=self.save_format_var, value="KRM").pack(side=tk.LEFT, padx=5)
        ttk.Radiobutton(save_fmt_frame, text="DWBA", variable=self.save_format_var, value="DWBA").pack(side=tk.LEFT, padx=5)

        # Save Shape Button
        self.save_shape_btn = ttk.Button(self.regular_ui_frame, text="Save Shape to TOML", command=self.save_generated_shape_to_toml, state=tk.DISABLED)
        self.save_shape_btn.pack(fill=tk.X, pady=(2, 0))


        # View Mode Selection
        ttk.Label(self.left_pane, text="View Mode:").pack(anchor=tk.W, pady=(20, 5)) # Extra pad
        view_frame = ttk.Frame(self.left_pane)
        view_frame.pack(fill=tk.X)
        ttk.Radiobutton(view_frame, text="2D View", variable=self.current_view_mode, value="2D", command=self.refresh_plot).pack(side=tk.LEFT, padx=(0, 10))
        ttk.Radiobutton(view_frame, text="3D View", variable=self.current_view_mode, value="3D", command=self.refresh_plot).pack(side=tk.LEFT)

        # Grid Toggle Checkbox
        ttk.Checkbutton(self.left_pane, text="Show Background Grid", variable=self.show_grid_var, command=self.refresh_plot).pack(anchor=tk.W, pady=(5, 0))

        # Info
        ttk.Label(self.left_pane, text="Organism Information:").pack(anchor=tk.W, pady=(10, 5))
        self.info_text = tk.Text(self.left_pane, height=20, wrap=tk.WORD, state=tk.DISABLED, width=40)
        ...
        self.context_menu.add_command(label="Reset View", command=lambda: self.toolbar.home())

        # Bind Mouse Wheel for Zoom and Right Click for Menu
        self.canvas_widget.bind("<MouseWheel>", self.on_mouse_wheel)
        self.canvas_widget.bind("<Button-3>", self.show_context_menu) # Right click
        self.canvas_widget.bind("<Button-4>", self.on_mouse_wheel) # Linux scroll up
        self.canvas_widget.bind("<Button-5>", self.on_mouse_wheel) # Linux scroll down
    ...
    def fetch_web_shapes(self):
        ...
        def run_fetch():
            try:
                ...
                self.root.after(0, lambda idx=index: self.on_index_success(idx))
            except Exception as e:
                err_msg = str(e)
                self.root.after(0, lambda msg=err_msg: self.on_fetch_error(msg))
    ...
    def on_web_shape_selected(self, event=None):
        ...
        if not hasattr(self, 'web_shape_index') or display_name not in self.web_shape_index:
            return
        ...
        def run_detail_fetch():
            try:
                shape_entry = api.fetch_shape_data(specimen_id, progress_callback=progress_cb)
                self.root.after(0, lambda se=shape_entry, dn=display_name: self.on_detail_success(dn, se))
            except Exception as e:
                err_msg = str(e)
                self.root.after(0, lambda msg=err_msg: self.on_fetch_error(msg))
    ...
    def update_param_fields(self, event=None):
        ...
        def add_field(label_text, var_name, default_val):
            ...
            ttk.Label(f, text=label_text).pack(side=tk.LEFT, fill=tk.X, expand=True)
            var = tk.DoubleVar(value=default_val)
            ttk.Entry(f, textvariable=var, width=16).pack(side=tk.RIGHT, padx=5)
    ...
    def generate_regular_shape(self):
        try:
            ...
            if shape_type == "Sphere":
                ...
                shape_data = geometry.generate_prolate_spheroid(params['a'], params['L'])
                model_type = "KRM"
            elif shape_type == "Cylinder":
                shape_data = geometry.generate_cylinder(params['a'], params['L'])
                model_type = "KRM"
            elif shape_type == "Bent Cylinder":
                shape_data = geometry.generate_bent_cylinder(params['a'], params['L'], params['rho_c'])
                model_type = "DWBA"

            # Load into "all_shapes" pseudo-file entry
            self.all_shapes = {shape_data['name']: {'data': shape_data, 'model_type': model_type}}
    ...
    def save_generated_shape_to_toml(self):
        if rtoml is None:
            messagebox.showerror("Error", "rtoml library is not installed. Saving is disabled.")
            return

        selected_name = self.shape_selector.get()
        if not selected_name or selected_name not in self.all_shapes:
            messagebox.showwarning("No Shape", "Please generate a shape first.")
            return
        ...
        if target_format == "KRM":
            ...
            if source_type == "KRM":
                ...
                if len(x_dwba) > 1:
                    dist = np.sqrt(np.diff(x_dwba)**2 + np.diff(y_dwba)**2 + np.diff(z_dwba)**2)
            ...
            if output_shape["x"]:
                if source_type == "DWBA": 
                     ...
                     if len(x_arr) > 1:
                        length = np.sum(np.sqrt(np.diff(x_arr)**2 + np.diff(y_arr)**2 + np.diff(z_arr)**2))
                     else:
                        length = 0.0
                else:
                    length = max(output_shape["x"]) - min(output_shape["x"])
        ...
        # Dialog
        default_filename = selected_name.replace(" ", "_").replace("(", "").replace(")", "").replace("=", "").replace(",", "_") + ".toml"
        ...
        if file_path:
            try:
                ...
                messagebox.showinfo("Success", f"Shape saved to:\n{file_path}")
            except Exception as e:
    ...
    def load_shapes_from_file(self, file_path):
        ...
        try:
            ...
            for shape in shapes_list:
                ...
                if model_type:
                    self.all_shapes[name] = {'data': shape, 'model_type': model_type}
    ...
    def _refresh_plot_internal(self):
        try:
            ...
            if selected_name in self.all_shapes:
                ...
                model_type = shape_info.get('model_type', 'KRM') # Safe get
                ...
                if mode == "2D":
                    self.plot_shape_2d(shape_info['data'], shape_info['model_type'])
                else:
                    self.plot_shape_3d(shape_info['data'], shape_info['model_type'])
    ...
    def plot_shape_2d(self, shape_data, model_type):
        ...
        if model_type == 'KRM':
            ...
            if 'shapes' in shape_data:
                 ...
                 for s in raw_shapes:
                     # Ensure basics
                     if 'boundary' not in s: s['boundary'] = 'fluid-filled'
                     if 'anatomical_type' not in s: s['anatomical_type'] = 'body'

                     shapes_for_plot.append(s)
            plot_unit = "m" # Datastore uses meters

        if shapes_for_plot:
            # Custom plotting to fix coordinate axis issues
            # We do NOT use plot_shape_outline because it enforces inverted axes
            
            # 1. Determine Global Coordinate Transform based on Body
            x_mult = 1.0
            
            # Find body
            body_s = next((s for s in shapes_for_plot if s.get('anatomical_type') == 'body' and 'x' in s), shapes_for_plot[0])
            if 'x' in body_s:
            ...
            for s in shapes_for_plot:
                ...
                if 'facets_0' in s:
                    ...
                    if model_type == 'DATASTORE':
                        ...
                        # DWBA Local File Fix: Head is typically at max X, so flip to 0
                        if len(x) > 0:
                            x = np.max(x) - x
                    ...
                    # Close loop
                    ax_top.plot([x[0], x[0]], [y_top[0], y_bot[0]], color=c)
                    ax_top.plot([x[-1], x[-1]], [y_top[-1], y_bot[-1]], color=c)
                    ...
                    # Close loop
                    ax_side.plot([x[0], x[0]], [z_top[0], z_bot[0]], color=c)
                    ax_side.plot([x[-1], x[-1]], [z_top[-1], z_bot[-1]], color=c)

                elif ('mass_density' in s or 'categories' in s) and 'voxel_size' in s:
                     # Voxels
                         try:
                             ...
                             if data is None:
                                 print(f"Skipping voxels: valid 3D data not found in mass_density or categories.")
                                 continue

                             v_size = np.array(s['voxel_size']) * 100
                             
                             # FEM/PTDWBA Fix: Original (H, L, W) -> Target (L, W, H)
                             inner_model = shape_data.get('model_type', '')
                             if inner_model in ['FEM', 'PTDWBA'] or model_type in ['FEM', 'PTDWBA']:
                             ...
                             # Lateral (Side): Collapse Y (axis 1) -> (X, Z) = (L, H)
                             ...
                             ax_top.imshow(dorsal_proj, aspect='equal', origin='lower', extent=extent_top, cmap='viridis')
                             ax_side.imshow(lateral_proj, aspect='equal', origin='lower', extent=extent_side, cmap='viridis')
                             
                         except Exception as e:
                             print(f"Error plotting voxels: {e}")

            # Auto-scale limits with padding
            # We need to find the global bounds of all plotted data
            all_x, all_y, all_z = [], [], []
            for s in shapes_for_plot:
                # We need to apply the SAME transforms as above to get correct limits
                # This duplicates logic but is safer than trying to extract from plot objects
                
                # ... Re-apply logic simplified for bounds ...
                # Actually, simpler: The loop above already plotted everything.
                # We can let Matplotlib autoscale, but 'sharex' + 'equal' creates conflict.
                # Force update of limits based on data.
                pass 
        ...
        if shapes_for_plot:
            ...
            for s in shapes_for_plot:
                ...
                # --- Inside Loop: Update Bounds ---
                # We need to capture the *transformed* coordinates used for plotting
                # But since we plot immediately, we can just track the raw x/y/z after transform?
                # The logic below duplicates the transform for bounds calculation
                ...
                if 'facets_0' in s:
                    ...
                    if len(ty) > 0:
                     ...
                     # Voxel bounds (approx)
                     # For voxels, we plot using imshow with extent
                     # Extent X: 0 to lx.
                     # Extent Y: -ly/2 to ly/2.
                     # Extent Z: -lz/2 to lz/2.
                     try:
                         ...
                         lx, ly, lz = v[0]*vs[0], v[1]*vs[1], v[2]*vs[2]
                         # Check if x needs shift? In plot loop we use origin='lower' extent=[0, lx...]
            ...
            if g_min_x != float('inf'):
                ...
                # The shared X axis must be large enough to accommodate the aspect ratio of the "tallest" other axis
                # Matplotlib's 'datalim' might struggle with shared axes.
                # We can try to force a bounding box that is large enough.
                
                # Heuristic: Subplots are typically wider than they are tall (e.g., aspect ratio ~2.5:1).
                # If the object is tall (large span_z), fitting it into the short vertical space requires 
                # shrinking the scale (zooming out). Since X and Y/Z scales are locked (equal aspect),
                # zooming out means expanding the X-axis range.
                # We multiply span_z by a factor (e.g., 2.5) to ensure the X-axis expands enough 
                # to accommodate the vertical Z-span within the subplot's aspect ratio.
                
                max_span = max(span_x, span_y, span_z * 2.5)
                
                # Center X
                mid_x = (g_max_x + g_min_x) / 2
                ax_top.set_xlim([mid_x - max_span/2 * 1.1, mid_x + max_span/2 * 1.1])
                ...
                ax_top.set_ylim([mid_y - view_h_y/2 * 1.2, mid_y + view_h_y/2 * 1.2])
                
                mid_z = (g_max_z + g_min_z) / 2
                # Ensure Z view isn't too tight
                view_h_z = max(span_z, max_span / 5.0)
                ax_side.set_ylim([mid_z - view_h_z/2 * 1.2, mid_z + view_h_z/2 * 1.2])

            ax_top.set_title(f'{shape_data.get("vernacular_name", shape_data.get("name", "Unknown"))} (Dorsal)')
            ax_top.set_ylabel(f'Width (cm)', labelpad=10)
            
            # Smart Grid & Aspect: Disable grid and use 'auto' aspect for Voxel/Surface (CT/Mesh)
            # Enable grid and use 'equal' aspect for Outlines (KRM/SDWBA)
            # Note: We check for 'voxel_size' to distinguish 3D Voxel grids from 1D KRM material properties
            is_voxel_or_surface = any(
                (('mass_density' in s or 'categories' in s) and 'voxel_size' in s) or 
                ('facets_0' in s) 
                for s in shapes_for_plot
            )
            ...
            if is_voxel_or_surface:
                # Use 'box' adjustment to shrink the axes area to fit the image exactly (no white padding)
                # This makes it look like Figure_2.png where the image IS the frame.
                ...
                # For Outlines, use 'datalim' to allow flexible window resizing while keeping 1:1
            ...
            ax_side.set_xlabel(f'Length (cm)', labelpad=10)
            ax_side.set_ylabel(f'Height (cm)', labelpad=10)
    ...
    def plot_shape_3d(self, shape_data, model_type):
        ...
        if model_type == 'KRM':
            ...
            if len(x_b) > 0:
                ...
                self.ax.plot_surface(X, Y, Z, color='cyan', alpha=0.3, edgecolor='none')
                self.ax.plot_wireframe(X, Y, Z, color='cyan', rstride=5, cstride=5, alpha=0.2, linewidth=0.5)
            ...
            if len(x_sb) > 0:
                ...
                Z_sb = z_c_sb[:, np.newaxis] + r_z_sb[:, np.newaxis] * np.sin(theta)

                self.ax.plot_surface(X_sb, Y_sb, Z_sb, color='white', alpha=0.9, edgecolor='none')
            ...
            if len(x) > 0 and len(a) > 0:
                ...
                self.ax.plot_surface(X, Y, Z, color='cyan', alpha=0.4, edgecolor='k', linewidth=0.1)
            ...
            # 1. Determine Global Coordinate Transform based on Body
            x_mult = 1.0
            body_s = next((s for s in shapes if s.get('anatomical_type') == 'body' and 'x' in s), None)
            if body_s:
                bx = np.array(body_s['x'])
                if len(bx) > 0 and np.mean(bx) < 0:
                    x_mult = -1.0
            ...
            for s in shapes:
                # Color logic: Body is cool cyan, internal organs are warm orange
                ...
                if 'facets_0' in s:
                    try:
                        ...
                        if len(facets) > 50000:
                            ...
                            self.ax.scatter(x[::skip], y[::skip], z[::skip], c=color, alpha=alpha, s=1)
                        else:
                            # Use slightly higher alpha and edgecolor to reduce "holes" caused by depth sorting
                            self.ax.plot_trisurf(x, y, z, triangles=facets, 
                                                 alpha=alpha, color=color, 
                                                 edgecolor='none', shade=True,
                                                 antialiased=False) # Disable antialiased to fill tiny gaps
                ...
                # Check if Voxels
                if ('mass_density' in s or 'categories' in s) and 'voxel_size' in s:
                    try:
                        # Determine data source
                        data = None
                        if 'mass_density' in s:
                        ...
                        if d_max > d_min:
                            # Use 6% threshold to filter background while keeping flesh
                            ...
                            def get_surface_indices(binary_mask):
                                ...
                                n_sum = (padded[:-2, 1:-1, 1:-1].astype(int) + 
                            ...
                            if len(ix) > 0:
                                ...
                                ys = (iy - ny/2) * v_size[1]
                                zs = (iz - nz/2) * v_size[2]
                                z_mult = -1.0 if inner_model not in ['FEM', 'PTDWBA'] else 1.0
                                ...
                                norm = plt.Normalize(vmin=thresh_bg, vmax=d_max)
                                colors = plt.get_cmap('turbo')(norm(voxel_values))
                                alphas = ((voxel_values - thresh_bg) / (d_max - thresh_bg)) ** 1.2
                                colors[:, 3] = alphas * 0.6 + 0.1
                                
                                self.ax.scatter(xs, ys, zs * z_mult, c=colors, s=1, edgecolors='none', depthshade=True)
                ...
                if len(x) > 1 and len(w) > 0:
                    ...
                    X = np.tile(x[:, np.newaxis], (1, len(theta)))
                    Y = np.tile(y[:, np.newaxis], (1, len(theta))) + r_y[:, np.newaxis] * np.cos(theta)
                    Z = np.tile(z[:, np.newaxis], (1, len(theta))) + r_z[:, np.newaxis] * np.sin(theta)
                    
                    self.ax.plot_surface(X, Y, Z, color=color, alpha=alpha, edgecolor='none')
    ...
    def toggle_grid(self):
        ...
        for ax in self.fig.axes:
            # 3D Zoom logic
            if hasattr(ax, 'get_zlim'):
                # In modern Matplotlib, we zoom by adjusting the FOV or limits
                cur_xlim = ax.get_xlim()
                cur_ylim = ax.get_ylim()
                cur_zlim = ax.get_zlim()
                ...
                rel_x = (event.x - ax.bbox.xmin) / ax.bbox.width
                rel_y = (event.y - ax.bbox.ymin) / ax.bbox.height
                
                # Center the zoom on the data mid-point for 3D
                x_mid = sum(cur_xlim) / 2
                y_mid = sum(cur_ylim) / 2
                z_mid = sum(cur_zlim) / 2
